...
- **Edit an order:** on a *New* or *Confirmed* order (not yet sent to the courier) tap **Edit** to change the name, phone, area, address, payment, note, a size or quantity, remove an item or **+ Add a product**. Items already in the order keep the price the customer was quoted; added items use today's price. Delivery, promo discount, total and stock update when you save (you'll see a message if a size doesn't have enough stock).
- **Print:** **🖨 Invoice** prints an A4 invoice (logo, items, discount, total, cash to collect). **🏷 Label** prints a parcel label with a big name, phone, address and the amount to collect, sized for small 80 mm label/receipt printers (it also prints fine on A4; cut it out).
- **Export to Excel:** Orders → **Export to Excel** → pick the dates → **Download CSV**. It opens in Excel or Google Sheets (Bangla included) with one row per order: date, status, customer, items, pieces, totals, promo, courier and notes.
...
- **Sale prices:** Products → Edit → **Sale price** (and optionally **Sale ends**, the last day), then Publish. The site shows ~~৳3,650~~ ৳2,990 −18%, the shop gets an *On sale* filter, orders are charged the sale price, and the normal price returns by itself after the last day.
- **Sales:** Stats tab → *Sales*: revenue, average order, delivered / returned / cancelled, revenue per day, best sellers, sizes sold, cash on delivery vs bKash and promo codes, for Today / 7 / 30 / 90 days.
- **Risky customers:** every order shows that phone number’s history (e.g. *3 orders · 2 delivered · 1 returned*; ⚠️ if they returned parcels). Use the menu on the order to **Require bKash advance** (that number can only order with bKash) or **Block** it. Set refused parcels to **Returned**: like Cancelled, this puts the stock back, and it counts in the customer’s history. Telegram alerts also warn about past returns.
...
- **Delivery dates:** Texts & settings → **Delivery**: besides the charges, set the days it takes inside Chittagong (e.g. `1-2`) and outside (`2-4`), the **order cut-off hour** (orders after it, e.g. 17 = 5 pm, count from the next day) and whether Friday is a day off. Product pages, quick view and checkout then show e.g. *🚚 Order now → Chittagong: by Mon 28 Sep*, calculated for the day the customer is looking. Publish to apply.
- **Eid banner:** Texts & settings → **Eid banner**: pick the **last order day** and a name (e.g. *Eid-ul-Adha*). The top bar shows *🌙 Order by Sat 3 Oct for delivery before Eid-ul-Adha (7 days left)*, counts down, and disappears by itself after that day. Clear the date to remove it early.
- **Site theme:** Texts & settings → **Look** → *Light (white & orange)* or *Dark (black & orange)*, then Publish.
...
**Morning summary:** every day at 9 am (Bangladesh time) the connected phones get yesterday's numbers: orders, revenue, average order, visitors, best seller, delivered and cancelled/returned, what's waiting now (to confirm, to ship, on the way) and sizes with 2 or fewer left. Turn it on or off under **📣 Phone alerts** (the *Morning summary* tick), or tap **Send it now** to see one. It needs no extra setup; the schedule comes with the Worker (`wrangler.toml`).
